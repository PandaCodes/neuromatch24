...
def excitatory_uniform(shape=(1,), lower=0.0, upper=1.0):
    assert lower >= 0
    return nn.init.uniform_(nn.Parameter(torch.empty(shape)), a=lower, b=upper)


def inhibitory_uniform(shape=(1,), lower=-1.0, upper=0.0):
    assert upper <= 0
    return nn.init.uniform_(nn.Parameter(torch.empty(shape)), a=lower, b=upper)


def unsigned_uniform(shape=(1,), lower=-1.0, upper=1.0):
    return nn.init.uniform_(nn.Parameter(torch.empty(shape)), a=lower, b=upper)


def excitatory_constant(shape=(1,), value=1.0):
    return nn.Parameter(torch.full(shape, value))


def inhibitory_constant(shape=(1,), value=-1.0):
    return nn.Parameter(torch.full(shape, value))



def unsigned_constant(shape=(1,), lower=-1.0, upper=1.0, p=0.5):
    with torch.no_grad():
...
class SwimmerModule(nn.Module):
    ...
    def forward(
        ...
        for i in range(self.n_joints):
            bneuron_d = bneuron_v = torch.zeros_like(
                joint_pos[..., 0, None]
            )  # shape (..., 1)
            ...
            if self.include_head_oscillators and i == 0:
                if timesteps is not None:
                    ...
                    phase = (
                        self.timestep % self.oscillator_period
                    )  # in [0, oscillator_period)
...
class SwimmerActor(nn.Module):
    ...
    def forward(self, observations):
        ...
        joint_limit = (
            2 * np.pi / (self.action_size + 1)
        )  # In dm_control, calculated with n_bodies.
        ...
        if self.timestep_transform:
            ...
            timesteps = (timesteps - low_in) / (high_in - low_in) * (
                high_out - low_out
            ) + low_out
